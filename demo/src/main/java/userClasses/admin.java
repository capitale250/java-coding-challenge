...
public class admin extends User {
	public JsonObject signup(JsonObject data,LinkedHashMap<String,JsonObject> map) {
		...
		if(storedInfo!=null) {
			userBuilder.add("message", "user with that email arleady exist");
			root = rootBuilder.add("response", userBuilder).build();
			return root;
		}
		if(data.getString("pass").length()!=10) {
            userBuilder.add("message", "password  must be equal to 10 ").add("data", data);
            root = rootBuilder.add("response", userBuilder).build();
